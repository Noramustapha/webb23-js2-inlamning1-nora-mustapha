...
function getAndDisplayHighscores() {
  fetch('http://localhost:3000/api/highscores')
    ...
    .then((data) => {
      highscores.length = 0; // Rensa highscores-arrayen
      highscores.push(...data); // Lägg till de nya highscores från servern
      updateHighscoreList();
    })
    .catch((error) => {
...
function updateHighscoreList() {
  highscores.sort((a, b) => b.score - a.score); // Sortera highscores i fallande ordning
  highscoreList.innerHTML = ''; // Rensa highscore-listan
...
function getComputerChoice() {
  ...
  const weights = [0.1, 0.8, 0.1]; // Vikter för varje alternativ (anpassa efter önskad sannolikhet)
  
  // Generera ett slumpmässigt tal mellan 0 och 1
  const random = Math.random();
  
  // Välj ett alternativ baserat på vikterna och det slumpmässiga talet
  ...
  // Om något går fel (t.ex. om vikterna inte summerar till 1), välj ett slumpmässigt alternativ
...
// Lyssna på keydown-händelsen på inputfältet
nameInput.addEventListener('keydown', function(event) {
  if (event.key === 'Enter') {
    // Kalla på submitForm om Enter-knappen trycks
    submitForm();
  }
});
